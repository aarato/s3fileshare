...
resource "null_resource" "npm_install_clipboard_onconnect" {
  triggers = {
    package_json = filemd5("${local.clipboard_onconnect_source_dir}/package.json")
  }
  provisioner "local-exec" {
    command     = "npm install --omit=dev"
    working_dir = local.clipboard_onconnect_source_dir
  }
}
...
data "archive_file" "clipboard_onconnect" {
  ...
  depends_on  = [null_resource.npm_install_clipboard_onconnect]
}
...
resource "aws_iam_role_policy" "clipboard_onconnect_logs" {
  name = "lambda-logs-policy"
  role = aws_iam_role.clipboard_onconnect.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
